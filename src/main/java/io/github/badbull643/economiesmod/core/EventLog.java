...
public class EventLog {
     ...
     */
    private SequencedEvent parseLine(String line) {
        JsonObject obj = new JsonParser().parse(line).getAsJsonObject();

        SequencedEvent se = new SequencedEvent();
        se.seq = obj.get("seq").getAsLong();
        se.prevHash = obj.has("prevHash") ? obj.get("prevHash").getAsString() : GENESIS_HASH;
    ...
    private Class<? extends Event> classFor(String typeName) {
        switch (typeName) {
